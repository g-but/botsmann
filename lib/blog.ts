    ...
    if (CONFIG.FORCE_CURRENT_DATE_FOR_PUBLISHED || !data.date) {
      if (CONFIG.VERBOSE_LOGGING) {
        console.log(
          ...
          data.date
            ? `Original date was: ${data.date}`
            : "No original date was provided.",
        );
      }
      return currentDate;
    }
    ...
    const posts = await Promise.all(
      directories.map(async (dir: any) => {
        ...
        if (data.featuredImage) {
          ...
          try {
            const imageExists = await fileExistsOnGitHub(
              `posts/${slug}/${imagePath}`,
            );
            if (!imageExists) {
              console.warn(
                `Featured image not found for ${slug}: ${featuredImage}`,
              );

              // Try alternative image formats
              const fileNameWithoutExt = imagePath.replace(
              ...
              for (const ext of ["jpg", "jpeg", "png", "webp", "jfif"]) {
                ...
                if (await fileExistsOnGitHub(altPath)) {
                  ...
                  console.log(
                    `Found alternative featured image for ${slug}: ${featuredImage}`,
                  );
                  break;
                }
              }

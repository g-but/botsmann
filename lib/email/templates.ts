import { EMAIL_ADDRESSES, EMAIL_URLS } from '@/lib/config/email';
...
export const getWelcomeEmailTemplate = (data: {
  name: string;
  preferences: { newsletter: boolean; productUpdates: boolean };
}): string => `
<!DOCTYPE html>
<html>
<head>
...
<body>
  <div class="container">
    <h1 class="heading">Welcome to Botsmann, ${data.name}!</h1>
    ...
    ${
      data.preferences.productUpdates
        ? '<p class="text">We\'ll keep you informed about new features and improvements that can benefit your business.</p>'
        : ''
    }
    <a href="${EMAIL_URLS.dashboard}" class="button">Visit Your Dashboard</a>
    <div class="footer">
      <p>Best regards,<br>The Botsmann Team</p>
      <p>Questions? Reply to this email or contact us at ${EMAIL_ADDRESSES.support}</p>
    </div>
  </div>
</body>
</html>
...
/**
 * Legacy template export for backwards compatibility
 * @deprecated Use getWelcomeEmailTemplate() instead
 */

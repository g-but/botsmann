  ...
  switch (inputType) {
    ...
    default:
      return {
        ...
        content: {
          message:
            "I couldn't determine the type of your input. Could you try with a word, phrase, or email?",
        },
      };
  }
};
    ...
    return [
      {
        iigabä: "de Hund, -",
        bedütig:
          "CH: de Hund DE: der Hund Es Tier mit vier Bei, wo böllt und treu isch.",
...
function translateSentence(sentence: string) {
  ...
  if (
    sentence.toLowerCase().includes("hello") ||
    sentence.toLowerCase().includes("hallo")
  ) {
    return {
...
function translateText(text: string) {
  ...
  return {
    hochdeutsch:
      "Dies ist eine Übersetzung des Textes auf Hochdeutsch. In einer realen Implementierung würde der gesamte Text übersetzt werden.",
    zuridutsch:
      "Das isch en Übersetztig vom Text uf Züridütsch. I de ächte Implementierig würd de ganz Text übersetzt werde.",
  };
}

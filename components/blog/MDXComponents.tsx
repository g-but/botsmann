...
const useAlternativeImageFormat = async (
  src: string,
): Promise<string | null> => {
  if (!src) return null;
...
const MDXComponents = {
  ...
  h1: (props: any) => (
    <h1
      className="mb-8 text-4xl font-semibold tracking-tight text-gray-900"
      {...props}
    />
  ),
  h2: (props: any) => (
  ...
  img: (
    ...
    if (!src) {
      ...
      return (
        <div className="my-8 p-4 bg-red-50 text-red-500">
    ...
    useEffect(() => {
      ...
      try {
        ...
        if (src.startsWith("http")) {
          ...
          console.log("Using dynamic slug for image path:", {
            contextSlug,
            imagePath,
            fullSrc,
          });
    ...
    if (!imageSrc) {
      return (
        <div className="my-8 p-4 bg-gray-50 text-gray-500">
          Loading image...
        </div>
      );
    }

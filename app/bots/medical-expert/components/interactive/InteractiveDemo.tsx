  ...
  const sendMessage = (message: string) => {
    ...
    setTimeout(() => {
      ...
      if (selectedCondition) {
        ...
        response =
          'Diabetes is a condition where your body has trouble regulating blood sugar. Type 2 diabetes is most common and often related to lifestyle factors. Would you like to learn about prevention, management, or potential complications?';
      } else if (
        message.toLowerCase().includes('anxiety') ||
        message.toLowerCase().includes('stress')
      ) {
        response =
          'Anxiety is a natural response to stress, but can become problematic when excessive. Evidence shows that cognitive behavioral therapy, mindfulness, and sometimes medication can be effective treatments. Would you like to know more about coping strategies?';
      } else {
        ...
        <div className="h-80 overflow-y-auto p-4 bg-gray-50">
          {chatHistory.length === 0 ? (
            <div className="text-center py-10 text-gray-500">
              <p className="mb-2">
                Select a health condition below or ask a question to start a conversation.
              </p>
            ...
            <div className="space-y-4">
              {chatHistory.map((msg, index) => (
                ...
                >
                  <div
                    className={`max-w-3/4 p-3 rounded-lg ${msg.sender === 'user' ? 'bg-blue-500 text-white' : 'bg-white border border-gray-200'}`}
                  >
                    {msg.message}
                  </div>
                </div>

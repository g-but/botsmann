...
interface TaxHistoryTabProps {
  taxHistory: TaxPayment[];
}

export const TaxHistoryTab: React.FC<TaxHistoryTabProps> = ({ taxHistory }) => {
  return (
    <div>
      <div className="mb-6">
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {taxHistory.map((tax) => (
                    <tr key={tax.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <span className={`px-2 py-1 inline-flex text-xs leading-5 font-semibold rounded-full ${getStatusColor(tax.status)}`}>
                          {tax.status}
                        </span>
                      </td>
